...
_INSTRUMENT_HINTS = {
    "AUTHORITY ENGINE":  ("recognition",   "Someone with decision-making power takes notice of your work today. Stay visible."),
    "REVENUE PIPELINE":  ("finance",        "An unexpected financial opening surfaces today. Stay alert to offers or information that arrives."),
    "ALLIANCE SYNC":     ("relationship",   "Someone significant re-enters your orbit today. The timing is not accidental."),
    "CAPITAL RUNWAY":    ("finance",        "A money-related decision or movement surfaces today — expected or not."),
    "CREATION ENGINE":   ("opportunity",   "An unexpected creative or strategic opening appears. Don't dismiss the first idea that comes."),
    "CONFLICT SHIELD":   ("warning",       "Something that needs attention surfaces today. Address it early — it gets heavier if ignored."),
    "REAL ESTATE RADAR": ("movement",      "A change in your physical environment or living situation comes into focus today."),
    "FORTUNE VECTOR":    ("opportunity",   "An opening arrives from an unexpected direction today. Stay receptive."),
    "GLOBAL VECTOR":     ("movement",      "A foreign connection or opportunity surfaces today — person, message, or idea."),
    "CAPITAL RESERVES":  ("finance",       "A financial pattern becomes clear today — track what moves, even small amounts."),
    "SYSTEM VITALS":     ("health",        "Your physical or mental energy signals something important today. Listen to it."),
    "ACTION CAPACITY":   ("decision",      "You will be asked to make a call you weren't expecting. Trust your first instinct."),
}

_STATUS_THRESHOLD = {"PEAK": 2, "ACTIVE": 1}  # 2=strong, 1=soft

def _get_wow_signal_for_chart(chart_id: str) -> dict:
    """
    Calls executive summary, finds highest-status instrument,
    returns event_signal dict or None.
    """
    try:
        exec_resp = supabase.table("charts").select("executive_summary").eq("id", chart_id).single().execute()
        exec_data = exec_resp.data.get("executive_summary") if exec_resp.data else None

        # If not cached, call the executive summary builder directly
        if not exec_data:
            ...
            if not chart_resp.data:
                return None
            exec_data = build_executive_summary(chart_resp.data, supabase)

        instruments = exec_data.get("instruments", [])
        if not instruments:
            return None

        # Find highest-status instrument
        best = None
        best_score = -1
        for inst in instruments:
            status = inst.get("signal_status", "")
            sig_score = inst.get("signal_score", 0)
            threshold = _STATUS_THRESHOLD.get(status, 0)
            if threshold > 0 and sig_score > best_score:
                best = inst
        ...
        status = best.get("signal_status", "")
        sig_score = best.get("signal_score", 0)

        # Find matching hint (partial match for instrument name)
        hint_data = None
        for key, val in _INSTRUMENT_HINTS.items():
            if key in inst_name or inst_name in key:
                hint_data = val
                break

        if not hint_data:
            hint_data = ("opportunity", "An unexpected signal surfaces today. Stay open to what arrives.")

        category, hint_text = hint_data

        strength = "STRONG" if status == "PEAK" else "ACTIVE"
...
async def get_daily_week(chart_id: str):
    ...
    Data used: Moon nakshatra/sign, Mercury sign, natal Moon sign.
    ...
    try:
        ...
        # 4. Get WOW event signal for TODAY only (expensive — only run once)
        wow_signal = _get_wow_signal_for_chart(chart_id)

        # 5. Inject WOW into today's entry only
        for i, day in enumerate(signals):
            if i == 0 and wow_signal:
