...
def create_stripe_checkout(
    ...
    try:
        ...
        # If no price ID configured yet — create one-time payment
        if not price_id:
            amount = PLAN_AMOUNTS_USD.get(plan_key, 499)
                ...
                line_items=[{
                    "price_data": {
                        ...
                        "product_data": {
                            ...
                            "description": "Vedic astrology life navigation",
                        },
                        "recurring": {"interval": "month"} if "monthly" in plan_key else None,
                    },
                    "quantity": 1,
                }],
                mode="subscription" if "monthly" in plan_key or "annual" in plan_key else "payment",
                success_url=success_url + "?session_id={CHECKOUT_SESSION_ID}",
                cancel_url=cancel_url,
                client_reference_id=chart_id,
                metadata={"chart_id": chart_id, "plan": plan_key},
            )
        else:
            session = stripe.checkout.Session.create(
                payment_method_types=["card"],
