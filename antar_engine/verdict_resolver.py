...
def build_fixed_facts_block(verdict: Dict[str, Any], language: str = "en") -> str:
    ...
    # [narration-polish] FIXED FACTS additions
    lines.append("  - NEVER describe a planet as an energy or trait.")
    lines.append("    Forbidden: any 'X energy' construction")
    lines.append("    ('structure-and-persistence energy',")
    lines.append("    'persistence energy', 'discipline energy').")
    lines.append("    Name the life-noun directly")
    lines.append("    (savings, holdings, a contract, the round, the deal).")
    lines.append("  - NEVER use metaphor verbs: 'pressing through',")
    lines.append("    'flowing through', 'moving through',")
    lines.append("    'coursing through'. No metaphor at all.")
    lines.append("  - NEVER use house jargon ('hidden gains',")
    lines.append("    'hidden losses', 'shared resources'). Plain:")
    lines.append("    'back-channel income', 'back-channel costs',")
    lines.append("    'joint funds'.")
    lines.append("  - State the favorable window as a CLOCK RANGE")
    lines.append("    or a single hard clock token from the")
    lines.append("    computed hora boundaries — NEVER a relative")
    lines.append("    phrase ('late morning', 'before midday',")
    lines.append("    'early', 'soon', 'later'). Examples:")
    lines.append("    'before 11:20 AM', 'after 5:06 PM',")
    lines.append("    '10:40 AM-12:10 PM'.")
    lines.append("  - Start every sentence with a capital letter.")
    if is_es:
