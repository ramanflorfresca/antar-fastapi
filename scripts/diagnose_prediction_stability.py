...
# [day8] DAILY config — /daily-week returns {days: [...] } where each
# day is independently generated by an LLM call.  We score stability
# PER DAY INDEX across runs (run_a.days[i] vs run_b.days[i]) then
# aggregate per field.
DAILY = {
    "route": "/api/v1/daily-week/{chart_id}?language=en&tz_offset=-5",
    "per_day_prose_fields": (
        "senal_de_hoy", "observa_hoy_text", "verdict_subline",
        "verdict_label", "wow", "el_movimiento",
    ),
    "per_day_list_fields": ("haz_hoy", "evita_hoy"),
    "per_day_enum_fields": ("verdict_emoji", "observa_hoy_domain"),
}
...
def analyze_daily(runs: list[dict]) -> list[str]:
    """
    /daily-week returns {days: [7 entries]}.  For each day index i,
    collect samples of each field across runs and score per-day
    stability.  Report per-field averages so the summary table
    mirrors analyze_monthly / analyze_annual shape.
    """
    out = ["", "## Daily-Week — per-field stability (averaged across 7 days)", "",
           "| field | type | stability | verdict |", "|---|---|---|---|"]
    if not runs:
        out.append("| (no successful runs) | | | |")
        return out

    # Number of days (use the min across runs in case of partial failures)
    n_days = min(len(r.get("days", []) or []) for r in runs)
    if n_days == 0:
        out.append("| (no days in response) | | | |")
        return out

    def _avg_per_day(field: str, kind: str) -> tuple[float, str]:
        per_day_scores = []
        for i in range(n_days):
            samples = [str((r["days"][i].get(field)) or "") for r in runs]
            if kind == "prose":
                s, _ = _stability_of_prose(samples)
            elif kind == "enum":
                s, _ = _stability_of_exact(samples)
            elif kind == "list":
                raw_lists = [r["days"][i].get(field) or [] for r in runs]
                s, _ = _stability_of_list(raw_lists)
            else:
                s = 0.0
            per_day_scores.append(s)
        avg = sum(per_day_scores) / len(per_day_scores) if per_day_scores else 0.0
        # Verdict buckets aligned with the other analyzers
        if avg >= 0.85:
            label = "STABLE"
        elif avg >= 0.55:
            label = "SEMI-STABLE"
    ...
    for f in DAILY["per_day_prose_fields"]:
        ratio, label = _avg_per_day(f, "prose")
        out.append(f"| {f} | prose | {ratio:.2f} | {label} |")
    for f in DAILY["per_day_list_fields"]:
        ratio, label = _avg_per_day(f, "list")
        out.append(f"| {f}[] | list | {ratio:.2f} | {label} |")
    for f in DAILY["per_day_enum_fields"]:
        ratio, label = _avg_per_day(f, "enum")
        out.append(f"| {f} | enum | {ratio:.2f} | {label} |")

    # Sample trios — pick day[0] (today) for visual inspection
    out.append("")
    out.append("### Day[0] sample trios (first 160 chars each run)")
    for f in ("senal_de_hoy", "observa_hoy_text", "verdict_subline", "el_movimiento"):
        out.append(f"\n**{f}:**")
        for i, r in enumerate(runs):
            if r.get("days"):
                v = str(r["days"][0].get(f) or "")[:160]
                out.append(f"  - run{i+1}: {v}")
    return out


# [day8] analyze_daily function
...
def main() -> int:
    ...
    # [day8] daily dispatch
    if "daily" in endpoints:
        print("\n▶ Daily Week")
        daily_runs = run_endpoint(args.base_url, DAILY["route"], args.chart_id, args.runs)
        if daily_runs:
            report.extend(analyze_daily(daily_runs))
